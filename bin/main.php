...
//	   $chkemail = $utilisateur->check_email($db);
      ...
      // Création automatique de l'hôtel
      $hotel = new THotel;
      ...
      if ($_REQUEST['hotel_name'] != "") {
      	$hotel->save($db);
      	$msg = "Votre établissement a été automatiquement créé, vous  pouvez commencer à créer vos chambres, vos clients et vos réservations en vous connectant";
      } else {
      	$msg = "Vous n'avez pas renseigné le nom de votre établissement, vous devrez donc le créer manuellement avec le bouton \"Créer un hôtel\"";
      }
			...
			if ($u->login($db,$login, $password)) {
			  ...
			  $_SESSION[SESS_GRP]->load($db, $u->id_groupe);  
				...
				if($_REQUEST['login']!='demo'){
				        setcookie("infos","Ceci permet à GHotel de vous identifier automatiquement", $time30d);
...
//  }
  if(isset($TErr['email']) && $TErr['email']=='erreur'){
      ?>
      <div class="information_erreur">
      L'e-mail que vous avez spécifié est soit invalide, 
      soit déjà utilisé. Veuillez le modifier en conséquence. Merci.
      </div>
      <?  
  }
...
<?	
  ...
  $t->Cell("Votre compte GHôtel",-1,'',2);
  $t->end_line();
...
//  $t->end_line();
  ...
  $t->beg_line();
  $t->Cell("E-mail");
  $t->Cell( $form->texte("","email",'auto',60,255,"title=\"E-mail valide auquel votre mot de passe sera envoyé\""));
  $t->end_line();
  
  $t->beg_line();
  $t->Cell("Société/groupe/organisme<br>(à défaut, votre nom de famille)");
  $t->Cell( $form->texte("","group_name",'auto',60,255,"title=\"Nom du groupe\""));
  $t->end_line();
  
  $t->beg_line("listheader");
  $t->Cell("Votre établissement",-1,'',2);
  $t->end_line();
  
  $t->beg_line();
  $t->Cell("Nom");
  $t->Cell( $form->texte("","hotel_name",'auto',30,150,"title=\"Nom de votre établissement\""));
  $t->end_line();
  
  $t->beg_line();
  $t->Cell("Adresse");
  $t->Cell( $form->texte("","hotel_adresse",'auto',50,150,"title=\"Adresse de votre établissement\""));
  $t->end_line();
  
  $t->beg_line();
  $t->Cell("Code postal");
  $t->Cell( $form->texte("","hotel_cp",'auto',10,10,"title=\"Code postal de votre établissement\""));
  $t->end_line();
  
  $t->beg_line();
  $t->Cell("Ville");
  $t->Cell( $form->texte("","hotel_ville",'auto',30,150,"title=\"Ville de votre établissement\""));
  $t->end_line();
