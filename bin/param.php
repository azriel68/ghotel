 ...
 */
	...
	entete("Gestion paramètres",'online');
...
<?
	...
	$t->end_line();
	$t->beg_line();
	$t->Cell($form->texte('Préfixe', 'TParam[0][prefixe]', $n->prefixe, 5,10) );
	...
	$t->end_line();
	$t->beg_line();
	$t->Cell($form->texte('Préfixe', 'TParam[1][prefixe]', $n->prefixe, 5,10) );
	...
	$t->end_line();
	$t->beg_line();
	$t->Cell($form->texte('Nombre de semaine à afficher', 'nb_semaine_planning', $h->get_parameter('nb_semaine_planning'), 3,10) );
	$t->Cell("");
	$t->Cell("");
	$t->Cell("");
	$t->end_line();
	...
	$t->beg_line("listheader");
	$t->Cell("Taxe de séjour",-1,'',4);
	$t->end_line();
	$t->beg_line();
	$t->Cell($form->texte('Montant de la taxe de séjour', 'taxe_sejour_mt', _f_prix($h->get_parameter('taxe_sejour_mt')), 5,10));
	$t->Cell("");
	$t->Cell("");
	$t->Cell("");
	$t->end_line();
