  ...
  switch($action){
	...
	switch($action){
        ...
        /*if(is_hotel_select()){
			...
			Félicitation, vous venez d'enregistrer les informations concernant votre hôtel. 
      Vous pouvez maintenant le gérer	à l'aide des boutons ci-dessus.
			</div>
			<?
      }
      ...
      if(get_hotel_access()) {
		...
		case 'NEW':
			...
			?>
			<div class="information">
			Une fois votre hôtel créé, vous pourrez ajouter rapidement et facilement 
      vos chambres, catégorie, produits et clients et commencer à utiliser votre 
      gestion hôtelière.
			</div>
			<?
		...
		case 'LIST':
			...
			*/
			...
			if($u->get_count_hotel($db)==0){
  			?>
  			<div class="information">
  			Pour gérer votre hôtel, vous devez préalablement le créer. Pour se faire cliquez 
  ...
  if($db->Get_recordCount()>0){
    ...
    foreach ($THotel as $key=>$id_hotel) {
    	...
    	$t->Cell(
         ...
         .$t->img("planning.gif","Accédez au planning")
         .' </a>'
         .'<br /><span class="mini">Réservation(s) : '.$hotel->get_count_reservation($db).'</span>'
         .'<br /><a href="'.$url_ref.'&url_go='.urlencode('../bin/reservation.php?action=NEW').'" class="mini">Nouvelle réservation</a>'
    	,-1,'','','center');
    	if(is_admin())$t->Cell(
    	   '<a href="'.$url_ref.'&url_go='.urlencode('../bin/chambre.php').'"> '
         .$t->img("chambre.gif","Gérer les chambres")
         .' </a>'
    	...
    	$t->Cell(
         ...
         .$t->img("contact.gif","Les clients de l'hôtel")
         .' </a>'
    	...
    	if(is_admin())$t->Cell(
    	   ...
    	   .'<br /><a href="'.$url_ref.'&url_go='.urlencode('../bin/model.php').'" class="mini">Les modèles de documents</a>'
    	   
         .'<br /><a href="'.$url_ref.'&url_go='.urlencode('../bin/produit.php').'" class="mini">Les produits/services</a>'
    	   .'<br /><a href="'.$url_ref.'&url_go='.urlencode('../bin/categorie.php').'" class="mini">Les catégories de chambres</a>'
    	   .'<br /><a href="'.$url_ref.'&url_go='.urlencode('../bin/utilisateur.php').'" class="mini">Les utilisateurs</a>'
    	   .'<br /><a href="'.$url_ref.'&url_go='.urlencode('../bin/param.php').'" class="mini">Les paramètres</a>'
    	,-1,'','','left');
