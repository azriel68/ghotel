...
function liste_reservation_du_client(&$c){
	...
	echo "<p align=\"center\">".$t->link("Ajouter une réservation","reservation.php?action=NEW&client=".$c->id,"button")."</p>";
	echo $lst->Render();
...
function liste_factures_du_client(&$c){
	...
	//requête
	$sql = "SELECT id as 'ID', numero as 'Numéro', dt_facture as 'Date',
			dt_cre as 'Création'
	FROM hot_facture
...
<?
	 ...
	 * Requête
	 */
	$sql = "SELECT id as 'ID',civ as 'Civilité',nom as 'Nom', prenom as 'Prénom'
			,adresse as 'Adresse',dt_cre as 'Création'
			FROM hot_client ";
  ...
  if(isset($_REQUEST['search'])){
    ...
    $lst->Str_trans('Prénom',$trans_s);
    $lst->Str_trans('Adresse',$trans_s);
*/
    $lst->highlight_search('Nom', $_REQUEST['search']);
    $lst->highlight_search('Prénom', $_REQUEST['search']);
...
//	echo $t->draw_index($_SERVER['PHP_SELF']."?", "hot_client;id_hotel=".get_sess_hotel_id(), "idx", $charIdx);
	...
	echo $t->link("Recherche (nom, adresse, tél., e-mail)","javascript:go_liste()","lien2");
