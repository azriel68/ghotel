...
$age2 = $_SESSION[SESS_HOTEL]->get_parameter("taxe_sejour_age2");
  ...
  ."<span style=\"font-size:9px;\"> (soit ".$resa->get_nbJour()." nuitée(s), départ réel le ".$resa->get_dtfin(true).")</span>"
   );
	$t->end_line();
	
	$t->beg_line();
	$t->Cell("Frais de réservation");
  ...
  $t->beg_line();
	...
	$t->Cell( "Conserver les prix saisis (négociation) ? "
  ."<span style=\"font-size:9px;\">Sinon le prix est celui indiqué dans la chambre</span>"
  );
	$t->end_line();
	...
	$t->beg_line();
	$t->Cell("Montant total de la taxe de séjour");
	$t->Cell(_f_prix($resa->get_taxe_sejour()));
	$t->end_line();
	$t->beg_line();
	$t->Cell("Frais supplémentaires");
...
function liste(){
	...
	echo "<p align=\"center\">";
  if(have_droit_sess_groupe()){
    echo $t->link("Ajouter une réservation","?actionform=NEW","button");
  }
  else{
    echo get_right_erreur_msg($t->link("Ajouter une réservation","#","button"));  
  }  
  echo "</p>";
	...
	//requête
	$sql = "SELECT a.id as 'ID', a.etat as 'Etat', b.num as 'Chambre', c.civ as 'Civilité', CONCAT(c.nom,' ',c.prenom) as 'Client'
