...
/**
 * Récupération ou création de la session facture
...
//$db->db->debug=true;
	...
	if($f->numero!="" && $f->dt_facture!="" && $f->nom_client!='') {
		if($f->id==0 || $f->check_num_facture($db)) {
			$f->save($db);
			info($f->get_numero()." ".(($f->type=='DEVIS')?"enregistré":"enregistrée"));
			return true;
		}
		else {
			erreur($f->get_numero()." existe déjà");
		}
	}
	else {
		erreur("Le numéro, la date et le client sont obligatoire");
	}
}
...
?>
  ...
  /* if($f->id_client>0 && $c->email!=''){
      ?><a href="javascript:retour()" class="button">Envoyer à <?=$c->email?></a><?
    }*/
  ?>
	<a href="javascript:retour()" class="button">Retour à <?=$f->get_numero()?></a>
  &nbsp;&nbsp;&nbsp;
