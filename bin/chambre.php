...
function enregistrer(&$db, &$c){
  ...
  if($c->id_categorie<=0 && $c->prix_of_categorie){
    erreur("Veuillez spécifier la catégorie de la chambre");
  }
  	...
  	$c->save($db);
		...
		erreur("Les champs prix ou catégorie ainsi que le numéro sont obligatoires");
	}
...
<?
	...
	//
	$t->Cell(
    $form->combo('','id_categorie', $c->TCategorie, $c->id_categorie,1,'chambre.show_new_categorie(this.value);','',array('0'=>"-- Choisir une catégorie --",'new'=>"-- Nouvelle catégorie --"))
    ."
    <div id=\"nouvelle_categorie\">
    <div class=\"titre\">Libellé</div><div class=\"chp\">".$form->texte('','new_categorie_lib','auto',70)."</div>
	...
	$t->Cell("<label for=\"prix_of_categorie\">Utiliser le tarif défini dans la catégorie ?</label><br><small>(ou définissez les prix ci-après)</small>");
	$t->end_line();
  ...
  $t->beg_tbl('formcadre',800,2,'','center');
	...
	$t->Cell("Tarif en &euro;<br><small> (ou tarif par défaut)</small>",120);
...
function liste($id_hotel){
  ...
  echo "</p>";
	...
	//requête
	$sql = "SELECT a.id as 'ID', LPAD(a.num,20,' ') as 'Numéro', b.libelle as 'Catégorie', a.nb_lit as 'Nombre de lits'
