...
entete("Sélectionner la chambre",'popup');
...
$lst->Set_hiddenColumn('Création', true);
...
if (!is_null($_REQUEST["FORM"]) && $_REQUEST["FORM"]>"")
    $lst->Set_onClickAction("set_chambre","'".$_REQUEST["FORM"]."'"); //on définie la fonction javascript appelée lors du click sur une ligne de la liste
...
function _collect_resa_for_date(&$db, &$lst,$time_deb, $time_fin){
 ...
 /* print_r($lst->lines);*/
  ...
  $lst->column['Après']=new TColumn;

  $lst->column['Avant']->type='none';
  $lst->column['Après']->type='none';

  $nb=count($lst->lines);
  for ($i=0; $i<$nb; $i++) {
   	/*
    	Moche, prévoir d'insérer dans la classe
    */
   	$ligne = & $lst->lines[$i];
